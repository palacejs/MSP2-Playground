...
function createStarfield(canvasId = 'starfield') {
  ...
  const stars = [];
  const connections = {};
  const STAR_COUNT = 120; // Arttırıldı
  const MAX_DISTANCE = 170; // Arttırıldı

  class Star {
    constructor() {
      this.x = Math.random() * canvas.width;
      this.y = Math.random() * canvas.height;
      this.vx = (Math.random() - 0.5) * 0.19;
      this.vy = (Math.random() - 0.5) * 0.19;
    ...
    update() {
      ...
      if (this.alpha >= 0.7 || this.alpha <= 0.1) this.alphaChange *= -1;
    }
  ...
  function drawLines() {
    for (let i = 0; i < stars.length; i++) {
      for (let j = i + 1; j < stars.length; j++) {
        ...
        const targetAlpha = dist < MAX_DISTANCE ? 0.55 - (dist / MAX_DISTANCE * 0.55) : 0;
